...
local debug_warn  = if getgenv().mstudio45_ESP_DEBUG then (function(...) warn("[mstudio45's ESP]", ...) end) else (function() end);
local debug_error = if getgenv().mstudio45_ESP_DEBUG then (function(...) error("[mstudio45's ESP] " .. table.concat({ ... }, " ")) end) else (function() end);
...
function Library:Add(espSettings: ESPSettings)
	...
	function ESP:Render()
		...
		-- // Global throttle // --
		local now = (os and os.clock and os.clock()) or tick()
		local rate = (Library.GlobalConfig and Library.GlobalConfig.UpdateRate) or 0
		if rate and rate > 0 then
			local nextUpdate = ESP._NextUpdate or 0
			if now < nextUpdate then
				return
			end
			ESP._NextUpdate = now + rate
		end
		...
		if Billboard then
			...
			if Billboard.Enabled then
				ESP._Cache = ESP._Cache or {}
				local cache = ESP._Cache
				local newText
				if Library.GlobalConfig.Distance then
					...
					newText = ESP.CurrentSettings.Name
				end
				...
				local newColor = (Library.GlobalConfig.Rainbow and Library.RainbowColor) or ESP.CurrentSettings.Color
				if cache.lastTextColor ~= newColor then
					BillboardText.TextColor3 = newColor
					cache.lastTextColor = newColor
				end

				local newSize = ESP.CurrentSettings.TextSize
				if cache.lastTextSize ~= newSize then
					BillboardText.TextSize = newSize
					cache.lastTextSize = newSize
				end
			end
		end
